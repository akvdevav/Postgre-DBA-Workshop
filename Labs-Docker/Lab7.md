...
### 🕵️‍♂️ Workshop Lab: The Great Crypto Heist (Docker PITR)
...
##### 🛠️ Step 1: Spin Up the Managed Docker Infrastructure
...
# Windows PowerShell
Remove-Item -Recurse -Force $env:USERPROFILE\pg_archive -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $env:USERPROFILE\pg_archive
```
...
docker run -d --name crypto-db \
  ...
  -v ~/pg_archive:/mnt/server_archive \
  # Windows users can use a full path such as C:\Users\<your-user>\pg_archive:/mnt/server_archive
  # Windows users can use a full path such as C:\Users\<your-user>\pg_archive:/mnt/server_archive
  postgres:latest \
  -c wal_level=replica \
  -c archive_mode=on \
...
Connect to the database from your local machine using your psql client:
...
```
docker exec crypto-db bash -c "echo 'host replication all all trust' >> /var/lib/postgresql/data/pg_hba.conf"
```

```
docker exec crypto-db bash -c "echo 'host replication all all trust' >> /var/lib/postgresql/18/docker/pg_hba.conf"
```

# 2. Reload the PostgreSQL configuration so the security rules apply instantly
```
docker exec crypto-db psql -U postgres -c "SELECT pg_reload_conf();"
```
...
# 4. Stream the backup data directly out of the database onto your host machine
```
pg_basebackup -h localhost -U postgres -D ~/pg_base_backup -Fp -Xs
# Windows PowerShell: use a path like C:\Users\<your-user>\pg_base_backup instead of ~/pg_base_backup
```
...
```
docker stop crypto-db && docker rm crypto-db
```
...
Run this entire block on your local terminal. Make sure to replace the timestamp below with your actual safe timestamp from Step 4:
...
# "restore_command = 'cp /mnt/server_archive/%f %p'" | Out-File -FilePath "$env:USERPROFILE\pg_base_backup\postgresql.auto.conf" -Encoding ASCII -Append
```
...
# Windows PowerShell equivalent:
# "recovery_target_time = '2026-06-07 07:15:22.123456+00'" | Out-File -FilePath "$env:USERPROFILE\pg_base_backup\postgresql.auto.conf" -Encoding ASCII -Append
```
...
# Windows PowerShell equivalent:
# "recovery_target_action = 'promote'" | Out-File -FilePath "$env:USERPROFILE\pg_base_backup\postgresql.auto.conf" -Encoding ASCII -Append
```
...
# Windows PowerShell equivalent:
# New-Item -Path "$env:USERPROFILE\pg_base_backup\recovery.signal" -ItemType File -Force
```

🚀 Step 7: Transition to a Native Volume & Launch Recovery
To prevent host filesystem permission issues from breaking the recovery process, we transfer our files into a native Docker volume and spin up the recovered server.

```
docker volume rm crypto_recovery_vol
```

# 1. Create a native volume managed purely inside the Docker host
```
docker volume create crypto_recovery_vol
```
...
docker run --rm -it \
  --user 0 \
  -v ~/pg_base_backup:/mac_backup \
  # Windows users can use a full path such as C:\Users\<your-user>\pg_base_backup:/mac_backup
  -v crypto_recovery_vol:/docker_vol \
  postgres:latest bash -c '
    BACKUP_DIR=$(dirname $(find /mac_backup -name PG_VERSION | head -n 1))
    cp -a $BACKUP_DIR/* /docker_vol/
    chown -R postgres:postgres /docker_vol
  '
```

# 3. Spin up our brand new recovery server using a custom, isolated PGDATA path
```
docker run -d --name crypto-recovered \
  -p 5432:5432 \
  -e POSTGRES_PASSWORD=crypto-secret \
  -e POSTGRES_HOST_AUTH_METHOD=trust \
